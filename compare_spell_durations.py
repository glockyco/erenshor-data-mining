...
for spell_id, (name, old_ticks, resource_name) in playtest_dict.items():
    ...
    if spell_id in main_dict:
        ...
        results.append((spell_id, name, old_ticks, old_secs, new_ticks, new_secs, ticks_changed, secs_diff, pct_change, resource_name))
    else:
        # Not in main (shouldn't happen based on earlier results)
        results.append((spell_id, name, old_ticks, old_secs, None, None, True, None, None, resource_name))
...
def print_table(rows, title):
    ...
    print(f"\n{title}")
    print("=" * 165)
    print(f"{'ID':<10} {'Spell Name':<35} {'Old(ticks)':<11} {'New(ticks)':<11} {'Old(sec)':<10} {'New(sec)':<10} {'Diff(sec)':<10} {'+Change':<10} {'ResourceName':<40}")
    print("=" * 165)
    ...
    for row in rows:
        ...
        print(f"{spell_id:<10} {name:<35} {int(ot):<11} {nt_str:<11} {os_str:<10} {ns_str:<10} {diff_str:<10} {pct_str:<10} {resource_name:<40}")

# Print both tables
print_table(ticks_changed, f"SPELLS WITH CHANGED TICK COUNTS ({len(ticks_changed)} spells)")
print_table(ticks_same, f"SPELLS WITH SAME TICK COUNTS ({len(ticks_same)} spells)")

# Print summary
print("\n" + "=" * 165)
print(f"Summary: {len(ticks_changed)} with changed ticks, {len(ticks_same)} with same ticks")
print(f"Old (5 sec/tick), New (3 sec/tick)")
