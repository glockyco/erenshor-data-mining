...
class TestAssetRipperInitialization:
    ...
    def test_init_auto_detect_from_path(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """Test automatic detection finds executable in PATH."""
        # Mock home to point to tmp_path where there are no common paths
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "empty_home")

        # Create executable in a test location
        executable = tmp_path / "bin" / "AssetRipper.GUI.Free"
        executable.parent.mkdir(parents=True)
        executable.touch()

        # Mock shutil.which to return our test executable
        def mock_which(name: str) -> str | None:
            if name == "AssetRipper.GUI.Free":
                return str(executable)
            return None

        monkeypatch.setattr("erenshor.infrastructure.assetripper.assetripper.shutil.which", mock_which)

        assetripper = AssetRipper(port=9000)

        assert assetripper.executable_path == executable

    def test_init_auto_detect_common_path(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """Test automatic detection finds executable in common paths."""
        # Mock home directory
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        # Create executable in common path
        common_path = tmp_path / "Projects/AssetRipper/AssetRipper.GUI.Free"
        common_path.parent.mkdir(parents=True)
        common_path.touch()

        # Patch shutil.which to return None (not in PATH)
        with patch("erenshor.infrastructure.assetripper.assetripper.shutil.which", return_value=None):
            assetripper = AssetRipper()

        assert assetripper.executable_path == common_path

    @patch("erenshor.infrastructure.assetripper.assetripper.shutil.which")
    def test_init_auto_detect_not_found(self, mock_which: MagicMock) -> None:
        """Test initialization fails when auto-detect can't find executable."""
        mock_which.return_value = None

        # Patch Path.exists to always return False
        with (
            patch.object(Path, "exists", return_value=False),
            pytest.raises(AssetRipperNotFoundError) as exc_info,
        ):
            AssetRipper()
