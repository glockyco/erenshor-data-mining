...
def test_generates_and_validates_lua_data_modules(tmp_path: Path) -> None:
    ...
    assert result.written_paths == [items_path, characters_path]
    assert result.validation_tools == {items_path: "stylua", characters_path: "stylua"}
    assert validated_paths == [items_path, characters_path]
