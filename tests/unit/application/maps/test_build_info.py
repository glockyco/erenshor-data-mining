...
def _write_inputs(tmp_path: Path) -> tuple[Path, Path]:
    ...
    (maps_dir / "static" / "tiles" / "TestZone" / "-1" / "0").mkdir(parents=True)
    ...
    (maps_dir / "static" / "tiles" / "TestZone" / "-1" / "0" / "0.webp").write_bytes(b"tile")
...
def test_tile_manifest_change_flips_only_tiles_group(tmp_path: Path) -> None:
    ...
    (maps_dir / "static" / "tiles" / "tiles-manifest.json").write_text(
        '{"zoom_levels": {"0": {"tiles": ["/tiles/TestZone/-1/0/0.webp"], "count": 2}}}\n'
    )
    after = build_info.compute_input_hashes(maps_source_dir=maps_dir, database_path=database_path)
...
def test_validate_tile_inputs_rejects_missing_directory(tmp_path: Path) -> None:
    with pytest.raises(build_info.TileInputError, match="directory is missing"):
        build_info.validate_tile_inputs(tmp_path / "maps")


def test_validate_tile_inputs_rejects_manifest_without_tiles(tmp_path: Path) -> None:
    maps_dir = tmp_path / "maps"
    tiles_dir = maps_dir / "static" / "tiles"
    tiles_dir.mkdir(parents=True)
    (tiles_dir / "tiles-manifest.json").write_text('{"zoom_levels": {}}\n')

    with pytest.raises(build_info.TileInputError, match="No WebP map tiles"):
        build_info.validate_tile_inputs(maps_dir)


def test_read_build_info_rejects_blank_hashes(tmp_path: Path) -> None:
    build_dir = tmp_path / "build"
    build_dir.mkdir()
    (build_dir / build_info.BUILD_INFO_NAME).write_text('{"code": "abc", "data": "", "tiles": "jkl"}\n')
