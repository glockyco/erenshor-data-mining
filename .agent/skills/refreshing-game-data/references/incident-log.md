...
- **`erenshor -V main maps build` is not a symlink restore tool**: it copies the DB into `src/maps/static/db/erenshor.sqlite` for production builds. Teardown scripts that want a live DB symlink should use `ln -snf` directly.
