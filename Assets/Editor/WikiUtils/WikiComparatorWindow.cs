...
public class WikiComparatorWindow : EditorWindow
{
    ...
    private void SetDefaultItemIdFromDb()
    {
        ...
        {
            Debug.LogWarning(
                $"[WikiComparatorWindow] Database not found at {_fullDbPathDisplay}. Cannot set default Item ID.");
        ...
        try
        {
            ...
            if (firstItemWithWiki != null)
            {
                ...
                _statusMessage =
                    $"Defaulting to first found Item ID '{_itemIdToCompare}'. Enter the exact Wiki Page Name (Item ID) to compare.";
        ...
        catch (Exception ex)
        {
            ...
            Debug.LogError(
                $"[WikiComparatorWindow] Error reading database to find default item: {ex.Message}\n{ex.StackTrace}");
        }
        finally
        {
            db?.Close();
    ...
    private void OnGUI()
    {
        ...
        if (!dbExists)
        {
            EditorGUILayout.HelpBox(
                $"Database file not found!\nPlease ensure the export has run via 'Tools > Database > Export Database' and the path is correctly set there.",
                MessageType.Error);
        }
    ...
    private async void CompareItemAsync()
    {
        ...
        try
        {
            ...
            if (result.AreEqual)
            {
                ...
                if (_displayLocalText != null &&
                    _displayLocalText.StartsWith("<")) // Check if local text is a status message
                {
                ...
                {
                    _statusMessage =
                        $"Match: Local template for '{itemRecord.Id}' matches the corresponding online tier.";
                    _statusMessageType = MessageType.Info;
                }
            }
            else // Not equal
            {
                ...
                if (result.ErrorMessage != null && (result.ErrorMessage.Contains("missing online") ||
    ...
    private ItemDBRecord? FindItemRecord(string itemId)
    {
        ...
        try
        {
            ...
            if (record == null)
            {
                ...
                _statusMessage =
                    $"Error: Item with ID '{itemId}' not found in the database '{Path.GetFileName(_fullDbPathDisplay)}'.";
                _statusMessageType = MessageType.Error;
        ...
        catch (SQLiteException ex)
        {
            ...
            _statusMessageType = MessageType.Error;
            Debug.LogError(
                $"[WikiComparatorWindow] SQLite error querying for item ID '{itemId}': {ex.Message}\n{ex.StackTrace}");
            return null;
        }
        catch (Exception ex)
        {
            ...
            _statusMessageType = MessageType.Error;
            Debug.LogError(
                $"[WikiComparatorWindow] General error querying for item ID '{itemId}': {ex.Message}\n{ex.StackTrace}");
            return null;
        }
        finally
        {
