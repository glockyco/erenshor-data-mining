<script lang="ts">
    ...
    interface Props {
        ...
        description?: string;
        /** Current path, e.g. `/zone-maps`. Drives canonical and og:url. */
        path: string;
        /** Open Graph type. `website` for almost everything; `article` for posts. */
        type?: 'website' | 'article' | 'profile';
        /** Path to the OG image. Defaults to the site-wide card. */
        image?: string;
        /** Alt text describing the OG image. */
        imageAlt?: string;
        /** Pixel dimensions of the OG image. Override when using a non-default image. */
        imageWidth?: number;
        imageHeight?: number;
...
<svelte:head>
    ...
    {#each ldArray as ld, index (index)}
        <svelte:element this={scriptTag} type="application/ld+json"
            >{JSON.stringify(ld)}</svelte:element
        >
    {/each}
</svelte:head>
