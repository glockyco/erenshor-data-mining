...
describe('Repository', () => {
	...
	it('gets character markers', async () => {
		const zone = 'Azure';
		const markers = await db.getCharacterMarkers(zone);
		expect(Array.isArray(markers)).toBe(true);
		expect(markers.length).toBeGreaterThan(0);
		expect(markers[0].category).toBe('character');
	});
	...
	it('gets spawn-point markers', async () => {
		const zone = 'Stowaway';
		const markers = await db.getSpawnPointMarkers(zone);
		expect(Array.isArray(markers)).toBe(true);
		expect(markers.length).toBeGreaterThan(0);
		// Function returns both 'spawn-point' and 'character' markers depending on spawn type
		const spawnPointMarker = markers.find(m => m.category === 'spawn-point');
		expect(spawnPointMarker).toBeDefined();
	});
