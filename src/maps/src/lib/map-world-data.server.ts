...
import type { RepositoryBase } from '$lib/database.base';
import {
    buildZoneConfigs,
...
export type MapWorldDataRepository = Pick<RepositoryBase,
    ...
    | 'getAchievementTriggerMarkers'
    | 'getDoorMarkers'
    | 'getItemBagMarkers'
    | 'getMiningNodeMarkers'
    | 'getSecretPassageMarkers'
    | 'getTeleportMarkers'
    | 'getTreasureLocMarkers'
    | 'getWaterMarkers'
    | 'getItemSources'
    | 'getAllItems'
> & {
...
export interface MapWorldDataBuilderOptions {
    databasePath?: string;
    repository?: MapWorldDataRepository;
}

export async function buildMapWorldData(
    options: MapWorldDataBuilderOptions = {}
) {
    const repo = options.repository ?? new Repository();
    try {
        await repo.init(options.databasePath ?? getMapsDatabasePath());
