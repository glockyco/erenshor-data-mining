<script lang="ts">
	...
	const config = $derived(MAPS[mapName]);
			...
			allMarkers.forEach((marker) => {
				...
				switch (marker.category) {
					case 'achievement-trigger':
						...
						break;
					case 'character':
						color = marker.isUnique ? 'green' : 'green';
						layer = marker.isUnique ? 'Characters (Unique)' : 'Characters (Common)';
						iconClass = 'fa-solid fa-user';
						if (!marker.isEnabled) color = 'gray';
						break;
					case 'door':
						color = 'brown';
						layer = 'Locked Doors';
				...
				const m = L.marker([marker.position.y - config.originY, marker.position.x - config.originX], {
					icon
				});
	...
	$effect(() => {
		...
		if (previousMapName !== null && previousMapName !== currentMapName && coordId) {
			goto('?', {
				replaceState: true,
				noScroll: true,
				keepFocus: true
