...
# Project directory can be passed as first positional argument
# This allows the script to be called from anywhere
if [[ -n "${1:-}" && "$1" != -* ]]; then
    PROJECT_DIR="$1"
    shift  # Remove project dir from arguments
else
    # Default: assume script is in the Unity project directory
...
USAGE:
    export.sh [PROJECT_DIR] [OPTIONS]

ARGUMENTS:
    PROJECT_DIR             Unity project directory (optional, defaults to script directory)
...
EXAMPLES:
    # Export all entities (from Unity project directory)
    ./export.sh -o erenshor.sqlite

    # Export from specific Unity project
    ./export.sh /path/to/unity/project -o erenshor.sqlite

    # Export specific entities
    ./export.sh -o game.sqlite -e items,spells,characters

    # Export from specific project with verbose logging
    ./export.sh /path/to/project -o output.sqlite -e items -l verbose
