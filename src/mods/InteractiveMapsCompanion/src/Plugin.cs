...
public sealed class Plugin : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        // Initialize config
        _configEnableLogging = Config.Bind(
            "Debug",
            "EnableLogging",
            false,
            "Enable/disable all logging output from this plugin."
        );
        _configSendInterval = Config.Bind(
            "Network",
            "SendInterval",
            0.1f,
        ...
        try
        {
            ...
            _server.Start(socket =>
            {
                socket.OnOpen = () =>
                {
                    ...
                    _logger.LogInfo(
                        $"WebSocket client connected. Total clients: {_allSockets.Count}"
                    );
                ...
                socket.OnClose = () =>
                {
                    ...
                    _logger.LogInfo(
                        $"WebSocket client disconnected. Total clients: {_allSockets.Count}"
                    );
                };
            });
    ...
    private void Update()
    {
        ...
        if (!(Time.time - _lastSendTime >= _configSendInterval.Value))
            return;
