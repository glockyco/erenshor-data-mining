...
public sealed class NavigationEngine
{
    ...
    private NavigationExplanation ApplySourceGateDetail(
        NavigationExplanation explanation,
        string? sourceKey
    )
    {
    ...
    private void Track(Vector3 playerPosition)
    {
        ...
        bool isSameScene =
            string.Equals(TargetScene, CurrentScene, StringComparison.OrdinalIgnoreCase)
            || TargetScene == null;
        if (isSameScene)
        {
    ...
    private NPC? FindLiveNpcBySpawnEdges(Vector3 playerPosition)
    {
        ...
        if (characterNode?.Type != NodeType.Character)
            return null;
