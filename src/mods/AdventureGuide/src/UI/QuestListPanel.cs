...
public sealed class QuestListPanel
{
    ...
    // Scene → zone display name lookup, built once from zone nodes
    ...
    public void Draw(float width)
    {
        ...
        if (count == 0)
        {
            ...
            if (_guide.NodesOfType(NodeType.Quest).Count == 0)
    ...
    private int DrawQuestList()
    {
        ...
        if (filterChanged || stateChanged)
        {
            ...
            _sorted.Clear();
            var all = _guide.NodesOfType(NodeType.Quest);
    ...
    private bool PassesSearch(Node quest)
    {
        ...
        if (quest.Zone != null && quest.Zone.Contains(term, StringComparison.OrdinalIgnoreCase))
            return true;

        // AssignedBy edges → NPC display names
        foreach (var edge in _guide.OutEdges(quest.Key, EdgeType.AssignedBy))
        {
            var target = _guide.GetNode(edge.Target);
            if (
                target != null
                && target.DisplayName.Contains(term, StringComparison.OrdinalIgnoreCase)
            )
                return true;
        }

        // RequiresItem edges → item display names
        foreach (var edge in _guide.OutEdges(quest.Key, EdgeType.RequiresItem))
        {
            var target = _guide.GetNode(edge.Target);
            if (
                target != null
        ...
        if (SearchStepEdges(quest.Key, EdgeType.StepTalk, term))
            return true;
        if (SearchStepEdges(quest.Key, EdgeType.StepKill, term))
            return true;
        if (SearchStepEdges(quest.Key, EdgeType.StepTravel, term))
            return true;
        if (SearchStepEdges(quest.Key, EdgeType.StepShout, term))
            return true;
        if (SearchStepEdges(quest.Key, EdgeType.StepRead, term))
            return true;
    ...
    {
        foreach (var edge in _guide.OutEdges(questKey, stepType))
        {
            var target = _guide.GetNode(edge.Target);
            if (
                target != null
    ...
    /// <summary>
    /// Resolves a scene name to its zone display name via zone nodes.
    /// </summary>
