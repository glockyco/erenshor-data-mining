...
public static class FrontierResolver
{
    ...
    /// <summary>
    /// Returns true when an implicit quest's entire actionable frontier is blocked
    /// by unlock requirements, making the quest uncompletable right now.
    ///
    /// Phase-dependent: for objective-phase frontiers any single blocked objective
    /// suppresses markers; for acceptance and turn-in phases all paths must be
    /// blocked before markers are suppressed.
    /// </summary>
    public static bool IsImplicitFrontierBlocked(QuestPlanProjection projection)
    {
        var frontier = projection.Frontier;
        if (frontier.Count == 0)
            return false;

        bool isObjectivePhase = frontier[0].IncomingLink.EdgeType
            is not (EdgeType.AssignedBy or EdgeType.CompletedBy);
    ...
    /// <summary>
    /// Returns true when a plan node is reachable only through unlock requirements,
    /// meaning it cannot be acted on without first satisfying a prerequisite.
    /// </summary>
    public static bool HasBlockingRequirement(QuestPlan plan, PlanNodeId nodeId, HashSet<PlanNodeId> visited)
    {
