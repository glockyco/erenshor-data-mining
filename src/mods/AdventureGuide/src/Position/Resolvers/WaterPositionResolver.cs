...
public sealed class WaterPositionResolver : IPositionResolver
{
    ...
    private readonly Dictionary<string, List<ResolvedPosition>> _cache = new(
        StringComparer.Ordinal
    );

    public WaterPositionResolver(CompiledGuideModel guide)
    {
        _guide = guide;
    }
    ...
    public void OnSceneLoaded()
    {
        ...
        foreach (var node in _guide.NodesOfType(NodeType.Water))
        {
    ...
    public void Resolve(Node node, List<ResolvedPosition> results)
    {
        ...
        if (node.X.HasValue && node.Y.HasValue && node.Z.HasValue)
            results.Add(new ResolvedPosition(node.X.Value, node.Y.Value, node.Z.Value, node.Scene));
    }

    private static List<ResolvedPosition> FindShorePoints(
        Vector3 center,
        float surfaceY,
        string scene
    )
    {
        ...
        for (int i = 0; i < DirectionCount; i++)
        {
            ...
            for (float d = StepSize; d <= MaxSearchDistance; d += StepSize)
            {
                ...
                // walkable position.
                var shorePos = NavMesh.SamplePosition(
                    hit.point,
                    out var navHit,
                    NavMeshSnapRadius,
                    NavMesh.AllAreas
                ...
                points.Add(new ResolvedPosition(shorePos.x, shorePos.y, shorePos.z, scene));
                break;
            }
        }
