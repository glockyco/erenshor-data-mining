...
internal sealed class MarkerProjector : IMarkerProjection
{
    ...
    private void RebuildEntries(MarkerCandidateList candidates)
    {
        ...
        foreach (var candidate in candidates.Candidates)
        {
            ...
            if (
                !selectedCandidates.TryGetValue(dedupeKey, out var existing)
        ...
        foreach (
            var candidate in selectedCandidates
                .Values.OrderBy(candidate => candidate.Priority)
                .ThenBy(candidate => candidate.PositionNodeKey, StringComparer.Ordinal)
        )
        {
            _entries.Add(
                new MarkerEntry(candidate)
                {
            ...
            case LiveSourceKind.Character:
                switch (snapshot.Occupancy)
                {
                    ...
                    case LiveSourceOccupancy.Dead:
                        return new MarkerLiveRenderState(
                            snapshot.RequiresZoneReentry ? MarkerLiveStatus.ZoneReentry
                    ...
                    case LiveSourceOccupancy.UnlockBlocked:
                        return new MarkerLiveRenderState(
                            MarkerLiveStatus.UnlockBlocked,
                            null,
                            0f,
            ...
            case LiveSourceKind.MiningNode:
                switch (snapshot.Occupancy)
                {
                    case LiveSourceOccupancy.Available:
                        return new MarkerLiveRenderState(
                            MarkerLiveStatus.MiningAvailable,
                            null,
                            0f,
                            null
                        );
    ...
    private void ApplyLiveRenderState(MarkerEntry entry)
    {
        ...
        if (!candidate.ApplyLifecycleOverlay)
        {
    ...
    private void BuildProjectedEntries()
    {
        ...
        for (int i = 0; i < _entries.Count; i++)
        {
            ...
            if (
                RequiresSourceUniqueLifecycleDedupe(entry)
    ...
    private static bool RequiresSourceUniqueLifecycleDedupe(MarkerEntry entry) =>
        entry.Candidate.ApplyLifecycleOverlay
    ...
    private static string BuildLifecycleDedupeKey(MarkerEntry entry) =>
        string.Join("|", new[] { entry.Scene, entry.SourceKey, entry.Type.ToString() });
        ...
        if (!candidate.ApplyLifecycleOverlay)
        {
            return string.Join(
                ...
                new[]
                {
                    ...
                    candidate.X.ToString(System.Globalization.CultureInfo.InvariantCulture),
                    candidate.Y.ToString(System.Globalization.CultureInfo.InvariantCulture),
                    candidate.Z.ToString(System.Globalization.CultureInfo.InvariantCulture),
                    candidate.SubText,
                }
            );
        }
