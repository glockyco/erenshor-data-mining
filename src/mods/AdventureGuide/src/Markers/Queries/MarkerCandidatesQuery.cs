...
public sealed class MarkerCandidatesQuery
{
    ...
    )
    {
        for (int i = 0; i < resolution.CompiledTargets.Count; i++)
        {
            ...
            bool applyLifecycleOverlay = !IsLootTarget(target.Semantic);
            var (x, y, z) = ResolveStaticPosition(
                positionNode,
                target.X,
                target.Y,
                target.Z,
                preferTargetPosition: !applyLifecycleOverlay
            );
    ...
    private void EmitQuestGiverCandidates(Node quest, string scene, List<MarkerCandidate> sink)
    {
        foreach (var blueprint in _guide.GetQuestGiversInScene(scene))
        {
            ...
            string? blockedRequirement = FindFirstMissingRequirement(
    ...
    private SpawnCategory ReadSpawnCategoryForPosition(Node positionNode, Node? targetNode)
    {
        ...
        if (
            positionNode.Type == NodeType.SpawnPoint
    ...
    private static (float X, float Y, float Z) ResolveStaticPosition(
        ...
        float? targetY,
        float? targetZ,
        bool preferTargetPosition = false
    )
    {
        if (preferTargetPosition && targetX.HasValue && targetY.HasValue && targetZ.HasValue)
    ...
    private static bool IsLootTarget(ResolvedActionSemantic semantic) =>
        semantic.ActionKind is ResolvedActionKind.LootCorpse or ResolvedActionKind.LootChest;
    ...
    private static string? ExtractBlockedReason(
        SpawnCategory category,
