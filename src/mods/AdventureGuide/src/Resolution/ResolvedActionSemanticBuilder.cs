...
internal static class ResolvedActionSemanticBuilder
{
		...
		bool ready)
	{
		bool saysKeyword = blueprint.Interaction.Kind == MarkerInteractionKind.SayKeyword
			&& !string.IsNullOrEmpty(blueprint.Interaction.Keyword);
		bool hasPayload = HasTurnInPayload(guide, quest);
		...
		NavigationGoalKind goalKind)
	{
		...
		{
			string keyName = guide.GetNode(targetNode.Node.KeyItemKey!)?.DisplayName ?? targetNode.Node.KeyItemKey!;
	...
	private static bool HasTurnInPayload(CompiledGuideModel guide, Node requestedNode) =>
		...
		&& (guide.OutEdges(requestedNode.Key, EdgeType.RequiresItem).Count > 0
			|| guide.OutEdges(requestedNode.Key, EdgeType.RequiresMaterial).Count > 0);
