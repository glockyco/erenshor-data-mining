...
internal static class ResolvedActionSemanticBuilder
{
    ...
    )
    {
        ...
        var actionKind = DetermineActionKind(guide, requestedNode, goalNode, targetNode);
        string? keywordText = goalNode.Keyword ?? targetNode.Keyword;
        string? payloadText = BuildPayloadText(guide, requestedNode, goalNode, actionKind);
        ...
        string? rationaleText = BuildRationaleText(guide, goalNode, targetNode, goalKind);
    ...
    {
        bool saysKeyword =
            blueprint.Interaction.Kind == MarkerInteractionKind.SayKeyword
            && !string.IsNullOrEmpty(blueprint.Interaction.Keyword);
        var markerType =
            blockedRequirement != null ? QuestMarkerKind.QuestGiverBlocked
            : blueprint.Repeatable ? QuestMarkerKind.QuestGiverRepeat
    ...
    )
    {
        bool saysKeyword =
            blueprint.Interaction.Kind == MarkerInteractionKind.SayKeyword
            && !string.IsNullOrEmpty(blueprint.Interaction.Keyword);
        bool hasPayload = QuestCompletionSemantics.HasTurnInPayload(guide, quest);
        ...
        return new ResolvedActionSemantic(
            ...
            saysKeyword ? ResolvedActionKind.SayKeyword
                : hasPayload ? ResolvedActionKind.Give
                : ResolvedActionKind.Talk,
            goalNodeKey: null,
            goalQuantity: null,
            keywordText: blueprint.Interaction.Keyword,
            payloadText: hasPayload ? FormatTurnInPayload(guide, quest) : null,
    ...
    )
    {
        ...
        {
            if (
                !string.IsNullOrEmpty(goalNode.Keyword) || !string.IsNullOrEmpty(targetNode.Keyword)
            )
                return ResolvedActionKind.SayKeyword;

            return HasTurnInPayload(guide, requestedNode)
        ...
        {
            EdgeType.StepTalk
            or EdgeType.AssignedBy
            or EdgeType.GivesItem when !string.IsNullOrEmpty(targetNode.Keyword) =>
                ResolvedActionKind.SayKeyword,
            EdgeType.StepTalk or EdgeType.AssignedBy or EdgeType.GivesItem =>
                ResolvedActionKind.Talk,
            EdgeType.StepShout when !string.IsNullOrEmpty(targetNode.Keyword) =>
    ...
    )
    {
        if (actionKind == ResolvedActionKind.Give)
            return FormatTurnInPayload(guide, requestedNode);
    ...
    {
        bool sameTarget =
            goalNode.NodeKey == targetNode.NodeKey && goalNode.EdgeType == targetNode.EdgeType;
        if (sameTarget)
            return null;

        if (
            targetNode.Node.Type == NodeType.Door
            && !string.IsNullOrEmpty(targetNode.Node.KeyItemKey)
        )
        {
            string keyName =
                guide.GetNode(targetNode.Node.KeyItemKey!)?.DisplayName
                ?? targetNode.Node.KeyItemKey!;
    ...
    private static QuestMarkerKind DetermineActiveMarkerKind(
        Node requestedNode,
        ResolvedNodeContext goalNode
    ) =>
        goalNode.EdgeType == EdgeType.CompletedBy
            ? (
                requestedNode.Repeatable
                    ? QuestMarkerKind.TurnInRepeatReady
                    : QuestMarkerKind.TurnInReady
            )
            : QuestMarkerKind.Objective;

    private static bool IsCompletionTarget(
        ResolvedNodeContext goalNode,
        ResolvedNodeContext targetNode
    ) => goalNode.EdgeType == EdgeType.CompletedBy && targetNode.EdgeType == EdgeType.CompletedBy;

    private static bool HasTurnInPayload(CompiledGuideModel guide, Node requestedNode) =>
        QuestCompletionSemantics.HasTurnInPayload(guide, requestedNode);

    private static string? FormatTurnInPayload(CompiledGuideModel guide, Node requestedNode)
    {
        if (requestedNode.Type != NodeType.Quest)
            return null;

        var parts = new List<string>();
        CollectPayloadParts(guide, requestedNode.Key, EdgeType.RequiresItem, parts);
        CollectPayloadParts(guide, requestedNode.Key, EdgeType.RequiresMaterial, parts);
        if (parts.Count == 0)
            return null;
    ...
    {
        var edges = guide.OutEdges(questKey, edgeType);
        for (int i = 0; i < edges.Count; i++)
        {
            var itemNode = guide.GetNode(edges[i].Target);
            if (itemNode == null)
                continue;

            int quantity = edges[i].Quantity ?? 1;
            parts.Add(quantity > 1 ? $"{itemNode.DisplayName} ({quantity})" : itemNode.DisplayName);
        }
    }
    ...
    private static NavigationGoalKind DetermineGoalKind(ResolvedNodeContext node)
    {
        ...
        return node.EdgeType switch
        {
            ...
            EdgeType.StepTalk or EdgeType.StepShout or EdgeType.AssignedBy =>
