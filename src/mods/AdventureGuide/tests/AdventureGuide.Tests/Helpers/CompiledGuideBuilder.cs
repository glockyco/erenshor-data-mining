...
public sealed class CompiledGuideBuilder
{
	...
	private sealed record SpawnPointDef(string Key, string? Scene, string? Zone, float X, float Y, float Z);
	...
	public CompiledGuideBuilder AddEdge(
		...
		int? amount = null)
	{
		_edges.Add(new EdgeDef(source, target, type, group, ordinal, quantity, keyword, note, amount));
		return this;
	}
	...
	public CompiledGuideModel Build()
	{
		...
		foreach (ZoneLineDef zl in _zoneLines) allKeys.Add(zl.Key);
		foreach (ZoneDef z in _zones) allKeys.Add(z.Key);
		foreach (SpawnPointDef sp in _spawnPoints) allKeys.Add(sp.Key);
		foreach (DoorDef d in _doors) allKeys.Add(d.Key);
		...
		for (int i = 0; i < _edges.Count; i++)
		{
			...
			edgeDataList.Add(new CompiledEdgeData
			{
				SourceId = sourceId,
				TargetId = targetId,
				EdgeType = (int)e.Type,
