...
public sealed class QuestStateTrackerImplicitTests
{
    ...
    public void ImplicitQuest_InCompletionScene_IsImplicitlyAvailable_NotActionable()
    {
        ...
        var graph = new TestGraphBuilder()
            ...
            .AddCharacter("character:npc", "Npc", scene: "Stowaway")
        ...
        Assert.True(implicitlyAvailable, "Quest should be implicitly available in its completion scene.");
        Assert.False(actionable, "IsActionable must not return true — no plan resolution should run.");
    ...
    public void ImplicitQuest_WhenActive_IsActive_NotImplicitlyAvailable()
    {
        ...
        Assert.True(harness.Tracker.IsActionable("QuestQ"),
            "Active quest must remain actionable.");
    }
}
