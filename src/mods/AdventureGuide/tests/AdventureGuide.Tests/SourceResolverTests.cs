...
internal sealed class SelectiveResolutionLiveState : IResolutionLiveState
{
    public Dictionary<string, LiveChestPosition> CorpseLootPositions { get; } =
        new(StringComparer.Ordinal);
    public Dictionary<string, IReadOnlyList<LiveChestPosition>> RotChestPositions { get; } =
        new(StringComparer.Ordinal);

    public bool CorpseContainsItem(Node spawnNode, string itemStableKey) =>
        CorpseLootPositions.ContainsKey($"{spawnNode.Key}|{itemStableKey}");

    public bool TryGetCorpsePositionWithItem(
        Node spawnNode,
        string itemStableKey,
        out LiveChestPosition position
    ) => CorpseLootPositions.TryGetValue($"{spawnNode.Key}|{itemStableKey}", out position);

    public IEnumerable<LiveChestPosition> GetRotChestPositionsWithItem(string itemStableKey) =>
        RotChestPositions.TryGetValue(itemStableKey, out var positions)
            ? positions
            : Array.Empty<LiveChestPosition>();
}

public sealed class SourceResolverTests
{
    [Fact]
    public void Accepted_with_required_item_in_corpse_and_rot_chest_emits_loot_targets_alongside_spawn_source()
    {
        var guide = new CompiledGuideBuilder()
            .AddItem("item:pelt")
            .AddCharacter("char:wolf")
            .AddSpawnPoint("spawn:wolf", scene: "Forest", x: 10f, y: 0f, z: 20f)
            .AddItemSource("item:pelt", "char:wolf", positionKeys: new[] { "spawn:wolf" })
        ...
        var liveState = new SelectiveResolutionLiveState();
        liveState.CorpseLootPositions["spawn:wolf|item:pelt"] = new LiveChestPosition(
            12f,
            0f,
            22f,
            "Forest"
        );
        liveState.RotChestPositions["item:pelt"] = new[]
        {
            new LiveChestPosition(30f, 0f, 40f, "Forest"),
        };
        ...
        Assert.True(guide.TryGetNodeId("spawn:wolf", out int spawnId));
        Assert.True(guide.TryGetNodeId("char:wolf", out int wolfId));
        Assert.Equal(3, targets.Count);
        Assert.Contains(
            targets,
            target =>
                target.Semantic.ActionKind == ResolvedActionKind.Kill
                && target.PositionNodeId == spawnId
                && !target.IsGuaranteedLoot
        );
        Assert.Contains(
            targets,
            target =>
                target.Semantic.ActionKind.ToString() == "LootCorpse"
                && target.PositionNodeId == spawnId
                && target.X == 12f
                && target.Z == 22f
                && target.IsActionable
