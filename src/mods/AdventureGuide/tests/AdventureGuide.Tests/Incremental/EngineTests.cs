...
public sealed class EngineTests
{
    ...
    [Fact]
    public void Tracer_IsCalledOnFailedRecompute()
    {
        var engine = new Engine<TestFact>();
        var query = engine.DefineQuery<int, int>(
            name: "Fails",
            compute: (_, key) => throw new InvalidOperationException($"bad:{key}"));
        var tracer = new RecordingTracer();
        engine.SetTracer(tracer);

        var ex = Assert.Throws<InvalidOperationException>(() => engine.Read(query, 7));

        Assert.Equal("bad:7", ex.Message);
        var failure = Assert.Single(tracer.Failures);
        Assert.Equal("Fails", failure.QueryName);
        Assert.Equal(7, failure.Key);
        Assert.Same(ex, failure.Exception);
        Assert.True(failure.Ticks >= 0);
        Assert.Empty(tracer.Recomputes);
    }
    ...
    private sealed class RecordingTracer : IEngineTracer<TestFact>
    {
        ...
        public List<(string QueryName, object Key, Exception Exception, long Ticks)> Failures { get; } = new();
        ...
        public void OnRecomputeFailed(string queryName, object key, Exception exception, long computeTicks) =>
            Failures.Add((queryName, key, exception, computeTicks));
