...
def compile(
    ...
    compiles it into the dense indexed format used for graph diagnostics,
    and writes guide.json.
    """
...
def export_mod(
    ...
    output: Path = typer.Option(
        ...
        "--output",
        "-o",
        help="Output path for quest-guide.json (default: quest_guides/quest-guide.json)",
    ),
    overrides: Path = typer.Option(
        None,
        "--overrides",
    ...
    """Export the embedded Adventure Guide quest-data wrapper.

    Reads the processed SQLite database, builds the entity graph, compiles it
    once, and writes the stable quest-guide.json wrapper consumed by the
    Adventure Guide mod.
    """
    cli_ctx: CLIContext = ctx.obj
