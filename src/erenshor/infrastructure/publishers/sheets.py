...
class GoogleSheetsPublisher:
    ...
    Features:
    - Batch writing for efficient uploads
    ...
    def publish(
        ...
        This method requires a Google Sheets table to exist on the target sheet.
        Tables are required to preserve filters and sorting when updating data.
        ...
        3. If no table exists:
           - Returns PublishResult with success=False and clear error message
           - User must create a table before deployment
        ...
        try:
            ...
            if not tables:
                # No table found - FAIL with clear error
                error_msg = (
                    f"No Google Sheets table found on sheet '{sheet_name}'. "
                    f"Please create a table:\n"
                    f"1. Select the data range in Google Sheets (including header row)\n"
                    f"2. Go to Format → Convert to table\n"
                    f"3. Ensure 'Headers' checkbox is checked\n"
                    f"Tables are required for deployment to preserve filters and sorting."
                )
        ...
        try:
            ...
            for sheet in spreadsheet_metadata["sheets"]:
                if sheet["properties"]["title"] == sheet_name:
                    ...
                    if "tables" in sheet:
                        tables: list[dict[str, Any]] = sheet["tables"]
    ...
    def _update_table_range(
        ...
        table_range: dict[str, Any],
        new_row_count: int,
        new_column_count: int | None = None,
        is_data_source_table: bool = False,
    ) -> None:
