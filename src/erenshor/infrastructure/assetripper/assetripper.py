...
class AssetRipper:
    ...
    Example:
        ...
        ...     executable_path=Path("~/AssetRipper/AssetRipper.GUI.Free"),
        ...     port=8080,
        ...     timeout=3600
        ... )
        >>> assetripper.extract(
        ...
        >>> # With custom timeout
        >>> assetripper = AssetRipper(timeout=7200)  # 2 hour timeout
        >>> assetripper.extract(source_dir=Path("game"), target_dir=Path("unity"))
    """
    ...
    def __init__(
        ...
        Args:
            executable_path: Path to AssetRipper executable. If None, uses default detection.
        ...
        if not self.executable_path.is_file():
            raise AssetRipperNotFoundError(f"AssetRipper path is not a file: {self.executable_path}")
    ...
    def _find_executable(self) -> Path:
        """Find AssetRipper executable using common paths.

        Returns:
            Path to AssetRipper executable.

        Raises:
            AssetRipperNotFoundError: If executable cannot be found.
        """
        # Try common installation paths
        common_paths = [
            Path.home() / "Projects/AssetRipper/AssetRipper.GUI.Free",
            Path.home() / "Tools/AssetRipper/AssetRipper.GUI.Free",
            Path("/opt/AssetRipper/AssetRipper.GUI.Free"),
        ]

        for path in common_paths:
            if path.exists() and path.is_file():
                logger.debug(f"Found AssetRipper at: {path}")
                return path

        # Try finding in PATH
        which_result = shutil.which("AssetRipper.GUI.Free")
        if which_result:
            logger.debug(f"Found AssetRipper in PATH: {which_result}")
            return Path(which_result)

        raise AssetRipperNotFoundError(
            "AssetRipper executable not found.\n"
            "Set executable_path parameter or install to a standard location:\n"
            "  - ~/Projects/AssetRipper/AssetRipper.GUI.Free\n"
            "  - ~/Tools/AssetRipper/AssetRipper.GUI.Free\n"
            "Download from: https://github.com/AssetRipper/AssetRipper/releases"
        )
