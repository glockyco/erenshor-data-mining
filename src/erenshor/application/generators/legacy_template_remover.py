...
Removed Entirely:
- {{Enemy Stats}} - Replaced by stat tables in new system
...
- {{Enemy}}
- {{Ability}}
- {{Mold}}          - Status uncertain, left unchanged for now
- {{Ability Books}} - Status uncertain, left unchanged for now
"""
...
class LegacyTemplateRemover:
    ...
    #
    # Note: Template names are case-insensitive in MediaWiki, but we use
    # canonical casing here for consistency.
    LEGACY_MAPPINGS: ClassVar[dict[str, str]] = {
        ...
        "Auras": "Item",
        # Note: {{Mold}} and {{Ability Books}} status uncertain, not migrated yet
    }

    # Templates to remove entirely (no replacement)
    #
    # These templates are deprecated and should be removed from wiki pages.
    # They're replaced by other systems (e.g., stat tables instead of {{Enemy Stats}}).
