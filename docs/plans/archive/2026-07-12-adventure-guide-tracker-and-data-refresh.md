...
- The shipping mod embeds `quest_guides/quest-guide.json` as a resource
  (`AdventureGuide.csproj:22`, loaded at `GuideData.cs:68`). The maintained
  artifact is generated by `uv run erenshor guide export-mod` from the
  compiled graph and currently contains 206 quest entries, including the 22
  Planar March quests and the arena/workflow entries. `uv run erenshor guide
  compile` emits the dense `quest_guides/guide.json`; `export-mod` is the
  shipping wrapper artifact path.
...
- Modify `src/mods/AdventureGuide/src/Data/GuideData.cs`: wrapper schema —
  `SpawnPoint` parses scene/x/y/z/night_spawn plus
  `spawn_upon_quest_complete_stable_key`, `is_directly_placed`, and
  `source_script` (`:191-216`); `CharacterQuestUnlocks` is parsed and used by
  marker gating (`:43-46`, `:165-166`).
...
Lifecycle invariants: completion-timer expiry runs from the plugin update path
  even when the tracker window is hidden or disabled; initial and character-load
  quest-state syncs call `TrackerState.PruneCompleted`; and completed tracked
  quests retain an Untrack affordance while Track remains hidden. These paths
  cover completions that bypass `GameData.FinishQuest` and saves loaded with
  stale tracked pins.
...
- [x] In-game verification (full restart, per mod AGENTS): complete a tracked
...
Root cause addressed: `SpawnPointBridge.GetState` classifies any absent
  directly-placed NPC as `DirectlyPlacedDead` (`SpawnPointBridge.cs:128-170`),
  so `EmitPerSpawnMarkers` now consults `GuideData.CharacterQuestUnlocks`
  before emitting ZoneReentry (`WorldMarkerSystem.cs:425-436`). Confirmed live
  cases: `Lucian Revald` and `Revan Gavault` (Stowaway, kill-unlock group for
  `MEETBASSLE`).
...
- [x] Quest-gated direct placements (`spawn_upon_quest_complete`) and
  scripted spawns are covered by Task 4's per-spawn metadata; this task's
  direct unlock lookup fixes the reported kill-unlock class.
- [x] Automated verification:
  `uv run pytest tests/unit/mods/test_adventure_guide_markers.py` passes;
  `uv run erenshor mod build --mod adventure-guide` succeeds.
- [x] In-game verification: a kill-unlock NPC absent in Stowaway shows no
...
- [x] Regenerate `quest_guides/quest-guide.json` from the playtest DB via the
  new command; verify the current artifact contains 206 entries, including all
  22 Planar March quests and the arena/workflow entries, and the unobtainable
  `quest:amethikeys` / `quest:clearingthebonepits` entries carried by the
  legacy wrapper baseline are excluded. Diff shared quests against the legacy
  artifact baseline to catch mapping regressions.
...
- [x] Automated verification:
  ...
  artifact.
- [x] In-game verification: the new quests appear in the guide list with
  steps.
...
- [x] Vault republish of the rebuilt DLL ships the data: Vault version
  `2026.713.0` is the latest available Adventure Guide package
  (`adventure-guide_2026.713.0.zip`), contains `AdventureGuide.dll`, and its
  published changelog includes the Planar March quest data and tracker/marker
  fixes.
