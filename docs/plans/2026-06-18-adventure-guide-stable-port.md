...
5. **Test surface swap.** Delete main's `src/mods/AdventureGuide/tests/AdventureGuide.Tests/` (tests the deleted rewrite) and bring the stable AG's tests (`tests/unit/mods/test_adventure_guide_*.py`). These are source-assertion tests — the established pattern for these mods (runtime C# can't run under pytest). Acceptable; do not invent a C# test project.
...
- **Data drift (mitigated).** Task 8 Step 1 is the deep field-level check. If main's `quest-guide.json` ever diverges from `_version: 5`, stop and add an adapter.
- **Lost rewrite features.** Diagnostics/incident bundles, reactive markers, compiled-guide perf, and the rewrite's C# test coverage are intentionally dropped for the stable implementation. Accepted.
