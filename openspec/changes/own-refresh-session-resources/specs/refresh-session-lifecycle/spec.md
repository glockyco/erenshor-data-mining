...
A command that temporarily replaces repository state SHALL record the prior state before mutation and restore that state on normal exit, interruption, and startup or runtime failure.

#### Scenario: Maps development replaces an existing database link

- **WHEN** `maps dev` temporarily points the maps database link at a selected variant
- **THEN** the command restores the prior link target when it exits

#### Scenario: Maps development starts without an existing link

- **WHEN** `maps dev` creates a database link at a path that was absent before startup
- **THEN** the command removes that link when it exits

#### Scenario: Database path contains an unmanaged regular file

- **WHEN** `maps dev` finds a regular file where it would create the temporary database link
- **THEN** startup fails before changing the file
- **AND** the error identifies the conflicting path
