...
- [x] 3.3 Run `uv run erenshor maps dev` with a non-main variant, interrupt it, confirm its port is released, and confirm the exact prior database-link state is restored.
- [x] 3.4 Simulate an identity mismatch in the recovery path and confirm that the candidate process is reported but not signaled.
